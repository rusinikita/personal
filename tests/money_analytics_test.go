...
func (s *IntegrationTestSuite) TestGetSpendingByCategory_Depth1() {
	...
	_, out, err := money.GetSpendingByCategory(ctx, nil,
		money.GetSpendingByCategoryInput{From: from, To: to, Depth: 1})
	require.NoError(s.T(), err)
...
func (s *IntegrationTestSuite) TestGetSpendingByCategory_Depth2() {
	...
	_, out, err := money.GetSpendingByCategory(ctx, nil,
		money.GetSpendingByCategoryInput{From: from, To: to, Depth: 2})
	require.NoError(s.T(), err)
...
func (s *IntegrationTestSuite) TestGetTopMerchants_Success() {
	...
	_, out, err := money.GetTopMerchants(ctx, nil,
		money.GetTopMerchantsInput{From: from, To: to, Limit: 10})
	require.NoError(s.T(), err)
...
func (s *IntegrationTestSuite) TestGetTopMerchants_LimitRespected() {
	...
	txs := []money.TransactionInput{}
	for i, m := range []string{"M1", "M2", "M3", "M4", "M5"} {
		txs = append(txs, money.TransactionInput{
	...
	_, out, err := money.GetTopMerchants(ctx, nil,
		money.GetTopMerchantsInput{From: from, To: to, Limit: 3})
	require.NoError(s.T(), err)
...
func (s *IntegrationTestSuite) TestComparePeriods_Success() {
	...
	_, out, err := money.ComparePeriods(ctx, nil, money.ComparePeriodsInput{
	...
	diffByCategory := map[string]money.CategoryDiff{}
...
func (s *IntegrationTestSuite) TestGetBudgetProgress_ActiveBudget() {
	...
	_, _, err := money.SetBudget(ctx, nil, money.SetBudgetInput{
	...
	_, out, err := money.GetBudgetProgress(ctx, nil,
		money.GetBudgetProgressInput{At: at})
	require.NoError(s.T(), err)
...
func (s *IntegrationTestSuite) TestGetBalance_Success() {
	...
	_, out, err := money.GetBalance(ctx, nil,
		money.GetBalanceInput{From: from, To: to})
	require.NoError(s.T(), err)
