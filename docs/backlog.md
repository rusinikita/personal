...
## 1. MCP tool for sending Telegram messages

The agent should be able to proactively message the user via Telegram (e.g. reminders, notifications about a finished background task, questions that need attention outside the current session).

**Why:** Today the agent can only reply within an active, user-initiated conversation turn. It has no way to reach the user asynchronously — e.g. after a long background task finishes, or when it needs to flag something between sessions. This closes that gap.

**Use cases:**
- Agent should be able to send a text message to the user via an MCP tool call
- Agent should be able to send a message with formatting (markdown/HTML) to the user
- User should be able to receive the message in the existing Telegram bot chat
- Agent should be able to send a message without an active user-initiated conversation (proactive notification)
- System should be able to resolve the correct Telegram chat ID for the user without requiring it as a manual parameter each time

## 2. Web interface for Money
...
## 3. Web interface for Progress

A **read-only** evolution of the existing `/web/progress` dashboard (`action/progress/dashboard_web.go`), reusing its overall look (streak grid, project/habit split panels) but without its current constraints: it's built as a fixed-size, black-and-white 100vw/100vh screenshot layout showing only the top 5 activities per panel.

**Why:** The current dashboard is optimized for a screenshot (fixed viewport, B&W, top-5-only), so it can't show every active activity, has no drill-down into a single project's history, and has no way to browse finished or not-yet-started projects. A free-scrolling, color, full-list version turns it into a real browsing surface instead of a snapshot.
