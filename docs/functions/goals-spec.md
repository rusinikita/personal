...
- **The two all-types goal pages group by category, embedded single-domain grids stay chronological**: `/web/goals` and `/web/goals/eink` both call `BuildGoalTiles(..., types=nil)` — the only two callers that ever ask for every `goal_type` mixed together — so `BuildGoalTiles` treats `types == nil` as the signal to additionally sort the goals it got back from `ListGoals` into four category buckets, in order: `activities` (`activity_occurrence_count`, `activity_streak_count`) → `money` (`money_saving`, `money_spend`) → `gym` (`exercise_max_weight`, `exercise_total_volume`) → `others` (`manual`), `id` ascending within a bucket. Money/Progress-browse/Workouts pass a non-nil `types` subset (their own 1-2 goal types) and are left exactly as `ListGoals` returned them — `ORDER BY starts_at`, unchanged — since a single-domain grid has nothing to group by category and the chronological order was never a problem there. This is a Go-side sort inside `BuildGoalTiles`, not a `ListGoals`/SQL change, so it needs no repository or mock changes
- **Goal tiles link to their underlying page, per `goal_type`, where one exists**: `toGoalTileData` sets `webui.GoalTileData.LinkURL` from whichever of `Category`/`ExerciseID`/`ActivityID` the goal's type populates — `money_spend` → `/web/money/transactions?category=...` (the same URL shape `action/money`'s `categoryLinkURL` builds, reconstructed inline in `action/goals` since `action/money` already imports `action/goals` for its embedded tile grid and importing back would cycle), `exercise_max_weight`/`exercise_total_volume` → `/web/workouts/{exercise_id}`, `activity_occurrence_count`/`activity_streak_count` → `/web/progress/browse/{activity_id}`. `money_saving` and `manual` get no link (`LinkURL: ""`) — `money_saving` has no `Category` of its own to filter by (unlike `money_spend`) and linking to the bare, unfiltered `/web/money` dashboard wasn't judged worth the special case; `manual` has no underlying page at all. This applies everywhere a tile is rendered — dedicated page, e-ink page, and all three embedded grids — since a working drill-down is useful regardless of which page shows the tile
...
sequenceDiagram
    ...
    Goals->>Goals: types == nil, so sort by category bucket<br/>(activities → money → gym → others), then id ascending<br/>(see Best Practices — skipped when types is a non-nil subset)
    Goals->>Goals: map each to webui.GoalTileData<br/>(Name, ProgressLabel, PercentComplete, Deadline, LinkURL, OverTarget)<br/>reading CurrentValue/RemainingValue() straight off the Goal
...
sequenceDiagram
    ...
    Goals->>Goals: types is a non-nil subset, so no category re-sort —<br/>keeps ListGoals' starts_at order (see Best Practices)
    Goals->>Goals: map to []webui.GoalTileData (LinkURL set for<br/>money_spend only — money_saving has no Category to link with)
...
Not a route — an exported Go function, the single place that turns a `Goal` into a `webui.GoalTileData`: calls `ListGoals(GoalFilter{UserID: userID, ActiveOnly: true, At: at, Types: types})` (a plain cached read, see Best Practices); when `types` is `nil` (the two all-types pages, `/web/goals` and `/web/goals/eink`), additionally re-sorts the result into the four category buckets described in Best Practices before mapping — a non-nil `types` subset skips this and keeps `ListGoals`'s `starts_at` order. For each goal it then formats `ProgressLabel`/`PercentComplete` per `goal_type` from its already-computed `CurrentValue`/`RemainingValue()` — pure display formatting, no further computation or DB calls — sets `Deadline` from `EndsAt` (empty if nil), sets `LinkURL` per `goal_type` (see Best Practices), and sets `OverTarget` when `CurrentValue > TargetValue` for `money_spend` only (the one type where exceeding is a bad thing). Called by `GET /web/goals` and `GET /web/goals/eink` (both with `types=nil`) and by the Money/Progress-browse/Workouts handlers (each with their own `types` subset) — see the "Embedded Goal Tiles" sequence diagram above.
